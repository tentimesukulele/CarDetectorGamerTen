...
void create_sci_widget(GtkWidget *window);
void set_syntax_highlighting(GtkWidget *sci);
...
void add_line_numbers(GtkWidget *sci);
void run_debugger(const char *filename);

// Global variables for widgets
static GtkWidget *sci; // Scintilla editor widget
...
void *load_file_async(void *filename) {
    ...
    // Now set the text to Scintilla widget in the main thread
    gdk_threads_add_idle((GSourceFunc)gtk_text_buffer_set_text, (gpointer)content);

    return NULL;
}

// Set up syntax highlighting for Python (just an example)
void set_syntax_highlighting(GtkWidget *sci) {
    // Syntax highlighting for Python (using Scintilla)
    // Set Python style
    SendMessage((HWND)sci, SCI_SETLEXER, SCLEX_PYTHON, 0);
    SendMessage((HWND)sci, SCI_STYLESETFORE, SCE_PY_COMMENT, RGB(0, 128, 0)); // Green for comments
    SendMessage((HWND)sci, SCI_STYLESETBOLD, SCE_PY_KEYWORD, 1); // Bold for keywords
    SendMessage((HWND)sci, SCI_STYLESETUNDERLINE, SCE_PY_STRING, 1); // Underline for strings
}

// Create the Scintilla widget
void create_sci_widget(GtkWidget *window) {
    // Create the Scintilla text editor widget
    sci = gtk_scintilla_new();
    gtk_widget_set_size_request(sci, 800, 600); // Size of the editor
    gtk_container_add(GTK_CONTAINER(window), sci);

    // Set syntax highlighting (for Python in this example)
    set_syntax_highlighting(sci);

    // Add line numbers
    add_line_numbers(sci);
}
...
void highlight_syntax_errors(const char *error_output) {
    ...
    while ((line_start = strstr(line_start, ":")) != NULL) {
        ...
        if (line_end) {
            ...
            // Set error marker for that line
            SendMessage((HWND)sci, SCI_MARKERADD, line_number - 1, 0); // -1 because Scintilla is 0-indexed
...
void add_line_numbers(GtkWidget *sci) {
    SendMessage((HWND)sci, SCI_SETMARGINWIDTHN, 0, 40); // Set margin width for line numbers
    SendMessage((HWND)sci, SCI_SETMARGINTYPE, 0, SC_MARGIN_NUMBER); // Set margin type to line numbers
    SendMessage((HWND)sci, SCI_SETMARGINMASKN, 0, SC_MASK_FOLDERS); // Set mask for foldable lines
}
...
int main(int argc, char *argv[]) {
    ...
    // Create the Scintilla editor widget
    create_sci_widget(vbox);
